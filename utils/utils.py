...
def assd_hd(pre, gt, spacing=(1, 0.1, 0.1)):
    sd = surface_distance.compute_surface_distances(gt > 0, pre > 0, spacing)
    hd = surface_distance.compute_robust_hausdorff(sd, 95.)
    assd = np.mean(surface_distance.compute_average_surface_distance(sd))
    return assd, hd
