...
from models.conv_lstm import SequenceWise2D
...
class AffineTransformer2D(nn.Module):
    ...
    def forward(self, src, mat, mode='bilinear'):
        src = src.float()
        norm = torch.tensor([[1, 1, src.shape[-2]], [1, 1, src.shape[-1]]], dtype=torch.float).cuda()
        norm = norm[np.newaxis, ...]
        mat_new = mat/norm  # 1, 2, 3
        grid = nnf.affine_grid(mat_new, [src.shape[0], 2, src.shape[-2], src.shape[-1]])
        return nnf.grid_sample(src, grid, mode=mode)
    
class AffineTransformerSequence(nn.Module):
    """
    2-D Affine Transformer
    """
    def __init__(self):
    ...
    def forward(self, src, mat, mode='bilinear'):
        n, t, c, h, w = src.shape
        src = src.float().reshape(-1,c,h,w)
        mat = mat.reshape(-1, 2, 3)
        norm = torch.tensor([[1, 1, h], [1, 1, w]], dtype=torch.float).cuda()
        norm = norm[np.newaxis, ...]
        mat = mat / norm
        if mat.shape[0] != src.shape[0]:
            mat = torch.tile(mat, (n*t, 1, 1))  # 1, 2, 3
        grid = nnf.affine_grid(mat, [src.shape[0], 2, src.shape[-2], src.shape[-1]])
        out = nnf.grid_sample(src, grid, mode=mode)
...
class SpatialTransformerSequence(nn.Module):
    def __init__(self):
        super(SpatialTransformerSequence, self).__init__()
    ...
    def forward(self, src, flow, mode='bilinear', padding_mode='zeros'):
        ...
        # revising here!!!
        # if len(flow.shape) > 3:
        #     grid = torch.tile(torch.unsqueeze(torch.unsqueeze(grid, 0)), (n, t, 1, 1, 1))  # add batch & ns-->(bs, ns, 2, h, w)
        # else:
        grid = torch.unsqueeze(grid, 0) # add batch
        grid = grid.type(torch.FloatTensor)
        ...
        new_locs = new_locs.permute(0, 2, 3, 1)[..., [1, 0]] # bs(1), h, w, dim(y, x)
        if new_locs.shape[0] != src.shape[0]:
            new_locs = torch.tile(new_locs, (n*t, 1, 1, 1))    # bs*ns, h, w, dim(x, y)

        out = nnf.grid_sample(src, new_locs, mode=mode, padding_mode=padding_mode)
        return out.view(n, t, c, h, w)
...
class SpatialTransform2D(object):
    def __init__(self, in_sequence, do_rotation=True, angle_z=(-np.pi / 12, np.pi / 12), 
                 do_scale=True, scale_x=(0.85, 1.15), scale_y=(0.85, 1.15),
                 do_translate=True, trans_x=(-0.15, 0.15), trans_y=(-0.15, 0.15),
                 do_shear=False, shear_xy=(-np.pi / 18, np.pi / 18), shear_yx=(-np.pi / 18, np.pi / 18),
                 do_elastic_deform=False, alpha=(0., 512.), sigma=(4., 10.)):
        self.do_rotation = do_rotation
        self.angle_z = angle_z
        self.do_scale = do_scale
        self.scale_x = scale_x
        ...
        if in_sequence:
            self.stn = SpatialTransformerSequence()
            self.atn = AffineTransformerSequence()
        else:
    ...
    def rand_coords(self, patch_size):
        coords = torch.from_numpy(self.create_zero_centered_coordinate_mesh(patch_size)).cuda()
        mat = torch.from_numpy(np.identity(2, dtype=np.float32)).cuda()
        if self.do_rotation:
            a_z = np.random.uniform(self.angle_z[0], self.angle_z[1])
            mat = self.rotate_mat(mat, a_z)
        if self.do_scale:
            sc_x = np.random.uniform(self.scale_x[0], self.scale_x[1])
            sc_y = np.random.uniform(self.scale_y[0], self.scale_y[1])
        ...
        ctr = torch.FloatTensor([patch_size[0] // 2, patch_size[1] // 2]).cuda()
        ...
        coords += ctr[np.newaxis, :, np.newaxis, np.newaxis] - grid[np.newaxis, ...]   # 1, 2, h, w

        mat = mat[np.newaxis, ...]  # 1, 2, 3
        return mat, coords

    def create_zero_centered_coordinate_mesh(self, shape):
        tmp = tuple([np.arange(i) for i in shape])
        coords = np.array(np.meshgrid(*tmp, indexing='ij'), dtype=np.float32)   # 2, h, w
        for d in range(len(shape)):
            coords[d] -= ((np.array(shape, dtype=np.float32) - 1) / 2.)[d]
        return coords[np.newaxis, ...]   # 1, 2, h, w

    def rotate_mat(self, mat, angle_z):
        rot_mat_z = torch.FloatTensor([[np.cos(angle_z), -np.sin(angle_z)], [np.sin(angle_z), np.cos(angle_z)]]).cuda()
        mat = torch.matmul(rot_mat_z, mat)
        return mat
    ...
    def scale_mat(self, mat, scale_x, scale_y):
        scale_mat = torch.FloatTensor([[scale_x, 0], [0, scale_y]]).cuda()
        mat = torch.matmul(scale_mat, mat)
        return mat

    def shear_mat(self, mat, shear_xy, shear_yx):
        shear_mat = torch.FloatTensor([[1, np.tan(shear_xy)], [np.tan(shear_yx), 1]]).cuda()
        mat = torch.matmul(shear_mat, mat)
        return mat

    def translate_mat(self, mat, trans_x, trans_y):
        trans = torch.FloatTensor([trans_x, trans_y]).cuda()
        trans = trans[:, np.newaxis]
        mat = torch.cat([mat, trans], dim=-1)
        return mat